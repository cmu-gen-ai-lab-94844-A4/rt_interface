...
<body>
    ...
    <div class="content">
      ...
      <div class="model-boxes">
        <button >Llama 1B (not instruct)</button>
        <button >Llama 3B (not instruct)</button>        
    ...
    async function getResponseFromServer(messageContent) {
  // Placeholder function to simulate fetching a response from the server
  try {
    const response = await fetch('/api/handle_message', {
      method: 'POST',
      headers: {
    ...
    return "Error in fetching response. Please check console.";
  }
}
